const API_URL = "https://script.google.com/macros/s/AKfycbwQoNUj3h6M925n4AgQjh2tIgYnQeXHsMM4eBJpPMXzQeAeQAqYVS436mSpiED3wYlu/exec";
...
function openLetter(row){
  ...
  $('letterDetailContent').innerHTML=`
    ...
    <div class="likes letter-likes"><button onclick="likeLetter(${l.row})">💙 좋아요 ${l.likes||0}</button><span>💬 댓글 ${(l.comments||[]).length}</span></div>
    <div id="comments" class="letter-comments">${(l.comments||[]).map(c=>`<div class="comment-bubble"><b>${esc(c.writer)}</b><span>${esc(c.text)}</span></div>`).join('')}</div>
    <div class="inline-form letter-comment-form"><input id="commentText" placeholder="댓글"><button onclick="addComment(${l.row})">전송</button></div>`;
  show('letterDetail');
}
