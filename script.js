const API_URL = "https://script.google.com/macros/s/AKfycbyH0dvRz2VW9TlbVRpmdArL59Ptq4pQIJgsh96vkWA08QdFjyqptBVgU9THzjZ-TYTz/exec";
...
function show(screen){document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active'));$(screen).classList.add('active');state.current=screen;document.body.classList.toggle('reading-letter',screen==='letterDetail');document.querySelectorAll('.tabbar button').forEach(b=>b.classList.toggle('active',b.dataset.screen===screen));const titles={home:['𖦹⋆彡🫧꩜♪⋆',''],letters:['편지','˖*♬˖ ࣪⊹'],write:['새 편지 쓰기',''],letterDetail:['편지','๑ˊૢᵕˋૢ๑'],calendar:['캘린더','날짜별 기록'],songs:['오노추','오늘 노래 추천'],week:['이번주','⋆⭒𓆟⋆｡˚𖦹𓆜✩⋆'],memories:['사진','사진 모아보기'],capsule:['타임캡슐','정해진 날에 열리는 편지'],settings:['설정','앱 관리']};$('screenTitle').textContent=titles[screen][0];$('screenSub').textContent=titles[screen][1];}
...
$('addSong').onclick=async()=>{const url=$('songLink').value.trim();if(!url)return;const temp={id:'temp_'+Date.now(),date:new Date().toISOString(),url,videoId:youtubeId(url),title:'제목 불러오는 중',pending:true};state.songs.unshift(temp);$('songLink').value='';renderSongs();const data=await api({type:'song-add',url});if(!data.ok){state.songs=state.songs.filter(s=>s!==temp);renderSongs();alert(data.message||'노래 추가에 실패했어요.');return;}loadAll()};
...
function bindSettings(){
  ...
  const info=$('appInfoBtn'); if(info) info.onclick=()=>setSettingMessage('교환일기 v1 / 예으니가 만드러씀 500번 수정해씀. / PWA 버전\n데이터는 Google Sheets와 Apps Script에 저장돼요.');
