const API_URL = "https://script.google.com/macros/s/AKfycbw8G3atxwNjAW4_sCXiMwIbF16FW3_-81BF5e-_dAJUCOsJbbcUzjf4GNNFKdO2tt9o/exec";
...
let state = {letters:[], songs:[], todos:[], memories:[], songSearchResults:[], current:'home', month:new Date(), selectedMood:'😀', letterPhoto:'', memoryPhoto:''};
...
function youtubeId(url=''){return (String(url).match(/(?:v=|youtu\.be\/|shorts\/)([\w-]+)/)||[])[1] || '';}
...
function show(screen){document.querySelectorAll('.screen').forEach(s=>s.classList.remove('active'));$(screen).classList.add('active');state.current=screen;document.body.classList.toggle('reading-letter',screen==='letterDetail');document.querySelectorAll('.tabbar button').forEach(b=>b.classList.toggle('active',b.dataset.screen===screen));const titles={home:['*✨ ੈ✩‧₊˚* ੈ⭐️‧₊','기록하기'],letters:['편지','𖦹⋆彡🫧꩜♪⋆'],write:['새 편지 쓰기','𖦹⋆彡🫧꩜♪⋆'],letterDetail:['편지','•ू♡'],calendar:['캘린더','날짜별 기록'],songs:['오늘의 노래 추천','🪼⋆.ೃ࿔*:･'],week:['이번주','하고 싶은 것'],memories:['✧˖°. ♪','사진 모아보기'],capsule:['타임캡슐','˚₊✩‧₊'],settings:['설정','앱 관리']};$('screenTitle').textContent=titles[screen][0];$('screenSub').textContent=titles[screen][1];}
...
$('saveLetterBtn').onclick=async()=>{
  ...
  if(!title||!content){$('letterStatus').textContent='제목과 내용을 적어주세요.';return}
  const typedSong=$('letterSong').value.trim();
  const songUrl=typedSong||extractYouTubeUrl(content);
  const payload={type:'letter-add',writer:$('writer').value,mood:state.selectedMood,title,content,photoUrl:state.letterPhoto,songUrl,openDate:$('isCapsule').checked?$('capsuleDate').value:''};
  const temp={row:'temp_'+Date.now(),date:new Date().toISOString(),writer:payload.writer,mood:payload.mood,title,content,photoUrl:payload.photoUrl,songUrl,openDate:payload.openDate,likes:0,comments:[],pending:true};
  const tempSong=songUrl?{id:'temp_song_'+Date.now(),date:new Date().toISOString(),url:songUrl,videoId:youtubeId(songUrl),title:'제목 불러오는 중',pending:true}:null;
  state.letters.push(temp);
  if(tempSong)state.songs.unshift(tempSong);
  $('letterTitle').value='';
  $('letterContent').value='';
  $('letterSong').value='';
  $('letterSongPreview').innerHTML='';
  state.letterPhoto='';
  $('letterPhotoPreview').classList.add('hidden');
  $('letterStatus').textContent='저장 중...';
  renderLetters();renderSongs();show('letters');
  const data=await api(payload);
  if(!data.ok){
    state.letters=state.letters.filter(x=>x!==temp);
    if(tempSong)state.songs=state.songs.filter(x=>x!==tempSong);
    renderLetters();renderSongs();show('write');
    $('letterStatus').textContent=data.message||'저장에 실패했어요.';
    return;
  }
...
function songCard(song,opts={}){
  const data=typeof song==='string'?{url:song}:song;
  const url=data.url||data.songUrl||'';
  if(!url)return '';
  const id=data.videoId||youtubeId(url);
  const title=data.title||data.songTitle||'제목 불러오는 중';
  const deleteBtn=opts.deletable?`<button class="song-delete" onclick="deleteSong('${esc(data.id||'')}',${Number(data.row||0)})">삭제</button>`:'';
  return `<div class="song-card song-player">
    <div class="song-info">
      <b>${esc(title)}</b>
      <p class="meta">오노추</p>
      ${deleteBtn}
    </div>
    ${id?`<iframe src="https://www.youtube.com/embed/${esc(id)}" title="${esc(title)}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>`:`<a href="${esc(url)}" target="_blank">▶ 듣기</a>`}
  </div>`;
}
$('addSong').onclick=async()=>{
  const value=$('songLink').value.trim();
  if(!value)return;
  if(extractYouTubeUrl(value) || youtubeId(value)){
    await addSongByUrl(value);
    return;
  }
...
async function searchSongs(query){
  ...
  box.innerHTML='<div class="meta search-status">검색 중...</div>';
  const data=await api({type:'youtube-search',query});
  if(!data.ok){box.innerHTML=`<div class="meta search-status">${esc(data.message||'검색에 실패했어요.')}</div>`;return;}
  const items=data.items||[];
  if(!items.length){box.innerHTML='<div class="meta search-status">검색 결과가 없어요.</div>';return;}
  state.songSearchResults=items;
  box.innerHTML=items.map(item=>`
    <button class="song-result" onclick="selectSongResult(${items.indexOf(item)})">
      <img src="${esc(item.thumbnail||'')}">
      <span><b>${esc(item.title)}</b><small>${esc(item.channelTitle||'')}</small></span>
    </button>
  `).join('');
}
async function selectSongResult(index){
  const item=state.songSearchResults[index];
  if(!item)return;
  const url=item.url || `https://www.youtube.com/watch?v=${item.videoId}`;
  await addSongByUrl(url,{title:item.title,videoId:item.videoId});
  $('songSearchResults').innerHTML='';
  state.songSearchResults=[];
}
async function addSongByUrl(url,meta={}){
  const temp={id:'temp_'+Date.now(),date:new Date().toISOString(),url,videoId:meta.videoId||youtubeId(url),title:meta.title||'제목 불러오는 중',pending:true};
  state.songs.unshift(temp);
  $('songLink').value='';
  renderSongs();
  const data=await api({type:'song-add',url,title:meta.title||'',videoId:meta.videoId||''});
  if(!data.ok){state.songs=state.songs.filter(s=>s!==temp);renderSongs();alert(data.message||'노래 추가에 실패했어요.');return;}
  loadAll();
}
function renderSongs(){const box=$('songList');box.innerHTML='';const list=[...state.songs].reverse();box.innerHTML=list.length?'':'<div class="item"><div class="meta">아직 오노추가 없어요.</div></div>';list.forEach(s=>box.insertAdjacentHTML('beforeend',songCard(s,{deletable:true})))}
async function deleteSong(id,row){
  if(!confirm('이 노래를 삭제할까요?'))return;
  const before=[...state.songs];
  state.songs=state.songs.filter(s=>String(s.id)!==String(id) && Number(s.row)!==Number(row));
  renderSongs();
  const payload={type:'song-delete',row};
  if(id && !String(id).startsWith('temp_')) payload.id=id;
  const data=await api(payload);
  if(!data.ok){state.songs=before;renderSongs();alert(data.message||'노래 삭제에 실패했어요.');}
}
